...
/**
 * Related case studies — white slider closing the detail page so it never
 * dead-ends (the byCrawford rule, as a track rather than a single next link).
 * Same-industry projects lead, the rest follow. Two cards per view on desktop
 * and one on phones; the arrows step a full view at a time. Cards are
 * media-forward (shot, client, one line of story) and link to a detail page
 * where one has shipped, to the live site until then, matching the index grid.
 * A full-width strip CTA closes the section out to the index.
 */
export function CaseStudyRelated({ currentSlug }: { currentSlug: string }) {
  ...
  return (
    <Section tone="light" frameClassName="!pb-0 !pt-12 md:!pt-20">
      ...
      >
        {items.map((c) => (
          <RelatedCard key={c.slug} study={c} />
        ))}
      </div>
...
function RelatedCard({ study: c }: { study: CaseStudyCard }) {
