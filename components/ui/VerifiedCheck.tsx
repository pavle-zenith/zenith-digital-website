...
/**
 * Scalloped verified badge — sits next to every testimonial name on the site.
 * Defaults to the dark-section ink; pass `className="text-light-text"` (or any
 * color token) on light surfaces.
 */
export function VerifiedCheck({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      // Default ink only when no tone is passed — cn is a plain join, so a
      // passed text-* class must not compete with a baked-in default.
      className={cn("h-[18px] w-[18px] shrink-0", className ?? "text-text")}
      fill="currentColor"
      aria-hidden
    >
